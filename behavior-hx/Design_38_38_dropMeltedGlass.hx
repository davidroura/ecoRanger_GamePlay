...
class Design_38_38_dropMeltedGlass extends ActorScript
{
	...
	override public function init()
	{
		...
		addWhenUpdatedListener(null, function(elapsedTime:Float, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled)
			{
				if((Engine.engine.getGameAttribute("glassAnimation") == 1))
				{
					actor.setAnimation("" + _animation1);
				}
				else if((Engine.engine.getGameAttribute("glassAnimation") == 2))
				{
					actor.setAnimation("" + _animation2);
				}
				else if((Engine.engine.getGameAttribute("glassAnimation") == 3))
				{
