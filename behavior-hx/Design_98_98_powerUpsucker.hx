...
class Design_98_98_powerUpsucker extends ActorScript
{
	...
	public var _whirlwindCollision:Bool;
	...
	public function new(dummy:Int, actor:Actor, dummy2:Engine)
	{
		...
		_suckingSpeed = 0.3;
		nameMap.set("whirlwindCollision", "_whirlwindCollision");
		_whirlwindCollision = false;
	...
	override public function init()
	{
		...
		addWhenUpdatedListener(null, function(elapsedTime:Float, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled)
			{
				if((Engine.engine.getGameAttribute("suckingPowerOn") && _whirlwindCollision))
				{
					_DistanceX = asNumber(((Engine.engine.getGameAttribute("playerXPos") + 25) - actor.getXCenter()));
					propertyChanged("_DistanceX", _DistanceX);
					_DistanceY = asNumber(((Engine.engine.getGameAttribute("playerYPos") + 30) - actor.getYCenter()));
					...
					actor.setVelocity(_Direction, _suckingSpeed);
					_suckingSpeed = asNumber((30 * 1.4));
		...
		{
			if(wrapper.enabled && sameAsAny(getActorType(219), event.otherActor.getType(),event.otherActor.getGroup()))
			{
				_whirlwindCollision = true;
				propertyChanged("_whirlwindCollision", _whirlwindCollision);
			}
		});
