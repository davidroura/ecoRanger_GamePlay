...
class Design_146_146_spawnHandlerBlock2 extends SceneScript
{
	...
	public function _customBlock_dropObstacle(__difficulty:Float):Void
	{
		...
		if((Engine.engine.getGameAttribute("playerDistance") >= _nextDropObstacle))
		{
			...
			if(((_trashType == 1) && (__difficulty > randomInt(Math.floor(-15), Math.floor(35)))))
			{
				_customEvent_dropMud();
			}
			_nextDropObstacle = asNumber((Engine.engine.getGameAttribute("playerDistance") + (randomInt(Math.floor(180), Math.floor(350)) / (__difficulty + 7))));
		...
		addWhenUpdatedListener(null, function(elapsedTime:Float, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled)
			{
				...
				if(!(_lifeWait))
				{
					...
					runLater(1000 * randomFloatBetween(1.6, 3.3), function(timeTask:TimedTask):Void
					{
