...
class SceneEvents_0 extends SceneScript
{
	...
	public var _tutorialStarted:Bool;
	
	/* ========================= Custom Event ========================= */
	public function _customEvent_tutorial():Void
	{
		if((!(Engine.engine.getGameAttribute("tutorialDone")) && !(_tutorialStarted)))
		{
			trace("" + "tutorial Started");
			_tutorialStarted = true;
			propertyChanged("_tutorialStarted", _tutorialStarted);
			runLater(1000 * 2, function(timeTask:TimedTask):Void
			{
				createRecycledActor(getActorType(198), 25, 100, Script.FRONT);
				Engine.engine.setGameAttribute("notificationText", "Glad you joined us. Grab the trash so we can recycle it.");
				createRecycledActor(getActorType(81), 170, 260, Script.FRONT);
			}, null);
			runLater(1000 * 6.5, function(timeTask:TimedTask):Void
			{
				createRecycledActor(getActorType(198), 25, 100, Script.FRONT);
				Engine.engine.setGameAttribute("notificationText", "Don't forget to use your power ups");
				createRecycledActor(getActorType(277), 15, 280, Script.FRONT);
				getLastCreatedActor().fadeTo(0 / 100, 4, Expo.easeOut);
			}, null);
	...
	public function new(dummy:Int, dummy2:Engine)
	{
		...
		nameMap.set("tutorialStarted", "_tutorialStarted");
	...
	override public function init()
	{
		...
		addWhenDrawingListener(null, function(g:G, x:Float, y:Float, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled)
			{
				...
				g.drawString("" + Engine.engine.getGameAttribute("playerDistance"), ((350 - getFont(190).font.getTextWidth((("" + Engine.engine.getGameAttribute("playerDistance")) + ("" + " METERS")))/Engine.SCALE) / 2), 40);
