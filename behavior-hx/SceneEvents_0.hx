		...
		addWhenDrawingListener(null, function(g:G, x:Float, y:Float, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled)
			{
				...
				if(Engine.engine.getGameAttribute("gameOver"))
				{
					...
					{
						g.drawString("" + (("" + _tempDistanceNum) + ("" + " METERS")), 50, 120);
					}
					else
					{
						g.drawString("" + (("" + Engine.engine.getGameAttribute("playerDistance")) + ("" + " METERS")), 50, 120);
					}
					g.setFont(getFont(201));
					g.drawString("" + (("" + "HIGHSCORE ") + ("" + Engine.engine.getGameAttribute("distanceHighScore"))), 90, 175);
