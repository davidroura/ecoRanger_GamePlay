		...
		addWhenUpdatedListener(null, function(elapsedTime:Float, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled)
			{
				if(((actor.getAnimation() == "drop") && (_dump == true)))
				{
					...
					{
						createRecycledActor(getActorType(131), (actor.getScreenX() + randomInt(Math.floor(0), Math.floor(10))), (actor.getScreenY() + randomInt(Math.floor(-10), Math.floor(15))), Script.FRONT);
					}
					_dump = false;
