...
class Design_95_95_spawnHandler extends SceneScript
{
	...
	public function _customEvent_selectObstacle():Void
	{
		if((!(_distanceObstacle == Engine.engine.getGameAttribute("playerDistance")) && ((Engine.engine.getGameAttribute("playerDistance") % _nextDistance) == 0)))
		{
			...
			_nextDistance = asNumber((Math.round((Engine.engine.getGameAttribute("ramdomUniversalNumber") * 500)) + 100));
	...
	override public function init()
	{
		...
		addWhenDrawingListener(null, function(g:G, x:Float, y:Float, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled)
			{
				...
				g.drawString("" + (("" + _distanceObstacle) + ("" + (("" + "%") + ("" + (("" + Engine.engine.getGameAttribute("playerDistance")) + ("" + (("" + "->") + ("" + (Engine.engine.getGameAttribute("playerDistance") % _nextDistance))))))))), 100, 100);
			}
		});
