...
class SceneEvents_16 extends SceneScript
{
	...
	public function _customEvent_randomSpawn():Void
	{
		runLater(1000 * randomFloatBetween(.01, 1.5), function(timeTask:TimedTask):Void
		{
			...
			{
				createRecycledActor(getActorType(131), -5, randomInt(Math.floor(130), Math.floor(440)), Script.FRONT);
			...
			{
				createRecycledActor(getActorType(137), -5, randomInt(Math.floor(130), Math.floor(440)), Script.FRONT);
			...
			{
				createRecycledActor(getActorType(139), -5, randomInt(Math.floor(130), Math.floor(440)), Script.FRONT);
	...
	override public function init()
	{
		...
		addWhenDrawingListener(null, function(g:G, x:Float, y:Float, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled)
			{
				...
				{
					engine.moveCamera((getScreenXCenter() + 1), getScreenXCenter());
					trace("" + "moving camera");
				...
				g.drawString("" + Engine.engine.getGameAttribute("total_cans"), 15, 10);
				g.drawString("" + Engine.engine.getGameAttribute("total_plasticBottles"), 75, 10);
				g.drawString("" + Engine.engine.getGameAttribute("total_glassBottles"), 135, 10);
			}
		});
		...
		addActorEntersRegionListener(getRegion(0), function(a:Actor, list:Array<Dynamic>):Void
		{
			...
			{
				createRecycledActor(getActorType(210), randomInt(Math.floor(200), Math.floor(300)), randomInt(Math.floor(125), Math.floor(200)), Script.FRONT);
			}
		});
		...
		addActorEntersRegionListener(getRegion(1), function(a:Actor, list:Array<Dynamic>):Void
		{
			...
			{
				createRecycledActor(getActorType(210), randomInt(Math.floor(200), Math.floor(300)), randomInt(Math.floor(230), Math.floor(310)), Script.FRONT);
			}
		});
		...
		addActorEntersRegionListener(getRegion(2), function(a:Actor, list:Array<Dynamic>):Void
		{
			...
			{
				createRecycledActor(getActorType(210), randomInt(Math.floor(200), Math.floor(300)), randomInt(Math.floor(340), Math.floor(420)), Script.FRONT);
			}
		});
