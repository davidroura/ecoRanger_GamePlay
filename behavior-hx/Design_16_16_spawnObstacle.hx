...
class Design_16_16_spawnObstacle extends SceneScript
{
	...
	override public function init()
	{
		...
		{
			if((Engine.engine.getGameAttribute("spawnThings") && !(_wait4Obstale)))
			{
				_wait4Obstale = true;
				propertyChanged("_wait4Obstale", _wait4Obstale);
		...
		runPeriodically(1000 * randomInt(Math.floor(10), Math.floor(15)), function(timeTask:TimedTask):Void
		{
			if((Engine.engine.getGameAttribute("spawnThings") && !(_wait4Obstale)))
			{
