...
class SceneEvents_8 extends SceneScript
{
	...
	public function _customEvent_createList():Void
	{
		...
		/* "these additions have to match upgradeButtonActor's animation names" */ Engine.engine.getGameAttribute("upgradeList").push("megaFuel");
		Engine.engine.getGameAttribute("upgradeList").push("planter");
	...
	public function _customEvent_fillUpgrades():Void
	{
		...
		for(index0 in 0...Std.int(Engine.engine.getGameAttribute("upgradeList").length))
		{
			if((index0 == 0))
			{
			...
			if((_upgradeCost[Std.int(index0)] > Engine.engine.getGameAttribute("totalPlastic")))
			{
				_currentButton = (("" + ("" + Engine.engine.getGameAttribute("upgradeList")[Std.int(index0)])) + ("" + "Gray"));
				propertyChanged("_currentButton", _currentButton);
			}
			else
			{
				_currentButton = ("" + Engine.engine.getGameAttribute("upgradeList")[Std.int(index0)]);
				propertyChanged("_currentButton", _currentButton);
			}
