...
class ActorEvents_204 extends ActorScript
{
	...
	override public function init()
	{
		...
		createRecycledActor(getActorType(55), 40, 65, Script.FRONT);
		createRecycledActor(getActorType(55), 130, 65, Script.FRONT);
		getLastCreatedActor().setAnimation("" + "boostOff");
		createRecycledActor(getActorType(55), 220, 65, Script.FRONT);
		...
		addWhenDrawingListener(null, function(g:G, x:Float, y:Float, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled)
			{
				if(!(Engine.engine.getGameAttribute("upgradeDescription") == 0))
				{
					...
					{
						g.drawString("" + Engine.engine.getGameAttribute("upgradeDescription"), 30, 130);
						g.setFont(getFont(250));
						g.drawString("" + "Better fuel efficiency", 30, 150);
					}
