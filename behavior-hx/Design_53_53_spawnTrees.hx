...
class Design_53_53_spawnTrees extends SceneScript
{
	...
	{
		createRecycledActor(getActorType(70), randomInt(Math.floor(-50), Math.floor(-30)), 0, Script.FRONT);
	...
	{
		createRecycledActor(getActorType(70), randomInt(Math.floor(230), Math.floor(270)), 0, Script.FRONT);
		getLastCreatedActor().makeAlwaysSimulate();
		getLastCreatedActor().setY(-200);
		_customEvent_treeType();
